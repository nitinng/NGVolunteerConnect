...
export default async function ModuleTasksView({ 
    moduleSlug, 
    projectId, 
    backHref, 
    taskHrefPrefix 
}: ModuleTasksViewProps) {
                ...
                {moduleTasks.map((task, idx) => {
                    ...
                    return (
                        <JourneyModuleCard 
                            ...
                            href={`${taskHrefPrefix}/${slugify(task.title)}`}
                        />
                    );
                })}
                {moduleTasks.length === 0 && (
                    <div className="col-span-full py-12 border-2 border-dashed rounded-xl text-center">
                        <p className="text-slate-500 font-medium">No tasks found in this module yet.</p>
                    </div>
                )}
            </div>
        </div>
