...
import { Plus, Trash2, Edit2, GripVertical, Settings, Search, Info, Target, ListTodo } from "lucide-react";
...
export default function OnboardingFlowBuilder({ projectId, title, description }: OnboardingFlowBuilderProps) {
    ...
    const loadData = async () => {
        setIsLoading(true);
        try {
            const mods = await getGeneralOnboardingModules(projectId);
            const tsks = await getGeneralOnboardingTasks(projectId);
    ...
    const handleSaveModule = async () => {
        ...
        try {
            const payload = { 
                ...
                type: projectId ? 'Project' as const : (editingModule.type || 'General' as const)
            };
            await upsertModule(payload as Partial<GeneralModule>);
    ...
    const handleDeleteModule = (id: string) => {
        toast("Are you sure? This deletes all associated tasks as well.", {
            action: {
                label: "Delete",
                onClick: async () => {
                    try {
    ...
    const handleSaveTask = async () => {
        ...
        try {
            await upsertTask(editingTask as Partial<GeneralTask>);
            toast.success(editingTask.id ? "Task updated" : "Task created");
            setEditingTask(null);
            loadData();
        } catch (e: any) {
    ...
    const handleDeleteBlock = (id: string) => {
        toast("Delete this content block?", {
            action: {
                label: "Delete",
                onClick: async () => {
                    try {
            ...
            <div className="relative overflow-hidden rounded-lg bg-slate-50 dark:bg-zinc-900/50 p-4 md:p-6 border border-slate-200 dark:border-zinc-800 shadow-sm">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">{title || "Onboarding Configuration"}</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium">
                                {description || "Manage the modular onboarding flow for this project."}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <TabsContent value="modules" className="mt-6 flex flex-col gap-6">
                    {editingModule ? (
                        <Card className="border-indigo-200 bg-indigo-50/10">
                            <CardHeader><CardTitle>{editingModule.id ? "Edit Module" : "New Module"}</CardTitle></CardHeader>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    {!projectId && (
                                        <div className="space-y-2">
                                            ...
                                            <Select value={editingModule.department_id || 'General'} onValueChange={v => setEditingModule({ ...editingModule, department_id: v === 'General' ? null : v, type: v === 'General' ? 'General' : 'Specific' })}>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select value={editingModule.icon || 'BookOpen'} onValueChange={v => setEditingModule({ ...editingModule, icon: v })}>
                                            ...
                                            <SelectContent className="max-h-[300px]">
                                                {['Target', 'GraduationCap', 'Heart', 'Puzzle', 'ListTodo', 'ShieldCheck', 'PlayCircle', 'BookOpen', 'UserCircle', 'MessageSquare', 'Users2'].map(id => (
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select value={editingModule.color || 'indigo'} onValueChange={v => setEditingModule({ ...editingModule, color: v })}>
                                            <SelectTrigger><SelectValue/></SelectTrigger>
                                            <SelectContent>
                                                {['indigo', 'rose', 'amber', 'emerald', 'blue', 'violet', 'orange'].map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                                            </SelectContent>
                                        </Select>
                                    </div>
                                </div>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {modules.map((mod, index) => (
                                    <TableRow key={mod.id} draggable onDragStart={() => setDraggedModuleId(mod.id)} onDrop={(e) => handleDropModule(e, mod.id)} onDragOver={(e) => e.preventDefault()}>
                                        <TableCell><GripVertical className="w-4 h-4 text-slate-400" /> {index + 1}</TableCell>
                                        <TableCell className="font-medium">{mod.title}</TableCell>
                                        <TableCell className="text-right flex items-center justify-end gap-1">
                                            <Button variant="ghost" size="icon" onClick={() => setEditingModule(mod)}><Edit2 className="w-4 h-4" /></Button>
                                            <Button variant="ghost" size="icon" className="text-red-500" onClick={() => handleDeleteModule(mod.id)}><Trash2 className="w-4 h-4" /></Button>
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                ...
                <TabsContent value="tasks" className="mt-6 flex flex-col gap-6">
                    {editingTask ? (
                        <Card className="border-emerald-200 bg-emerald-50/10">
                            <CardHeader><CardTitle>{editingTask.id ? "Edit Task" : "New Task"}</CardTitle></CardHeader>
                            <CardContent className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                        ...
                        <Button className="w-auto self-start gap-2 bg-emerald-600 text-white" onClick={() => setEditingTask({ icon: 'ListTodo' })}>
                    ...
                    {modules.map(mod => {
                        ...
                        return (
                            <div key={mod.id} className="mb-6 space-y-3">
                                <h3 className="font-bold flex items-center gap-2"><div className={`w-2 h-2 rounded-full bg-${mod.color}-500`} /> {mod.title}</h3>
                                        ...
                                        <TableBody>
                                            {modTasks.map(task => (
                                                <TableRow key={task.id}>
                                                    ...
                                                    <TableCell className="text-right flex items-center justify-end gap-1">
                                                        <Button variant="ghost" size="icon" onClick={() => setEditingTask(task)}><Edit2 className="w-4 h-4" /></Button>
                                                        <Button variant="ghost" size="icon" className="text-red-500" onClick={() => handleDeleteTask(task.id)}><Trash2 className="w-4 h-4" /></Button>
                                                    </TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                    ...
                    {!editingContentId ? (
                                                    ...
                                                    {modTasks.map(task => (
                                                        <TableRow key={task.id}>
                                                            ...
                                                            <TableCell className="text-right">
                                                                <Button variant="ghost" size="sm" className="text-blue-600" onClick={() => startEditingContent(task.id)}>
                                                                    <Edit2 className="w-3 h-3 mr-2" /> Edit Blocks
                                                                </Button>
                                                            </TableCell>
                                                        </TableRow>
                                                    ))}
                        ...
                        <div className="flex flex-col gap-6">
                            <div className="flex items-center justify-between pb-4 border-b">
                                ...
                                <Button variant="outline" size="sm" onClick={() => setEditingContentId(null)}>Close</Button>
                            </div>

                            <div className="space-y-4">
                                {contentBlocks.map((block) => (
                                    <div key={block.id} className="bg-white border rounded-lg p-4 flex items-center justify-between shadow-sm">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="px-2 py-0.5 rounded-full bg-slate-100 text-[10px] font-bold uppercase">{block.type}</span>
                                            </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Button variant="ghost" size="sm" className="text-red-500" onClick={() => handleDeleteBlock(block.id)}><Trash2 className="w-4 h-4" /></Button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="mt-8 border-t pt-8">
                                {editingBlock ? (
                                    <Card className="border-blue-200 bg-blue-50/20">
                                        <CardHeader><CardTitle>{editingBlock.id ? "Edit Block" : "Add Block"}</CardTitle></CardHeader>
                                        <CardContent className="space-y-4">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <Select value={editingBlock.type || ''} onValueChange={v => setEditingBlock({ ...editingBlock, type: v, metadata: {} })}>
                                                        ...
                                                        <SelectContent>
                                                            ...
                                                            <SelectItem value="reflection_question">Reflection</SelectItem>
                                                        </SelectContent>
                                                    </Select>
                                                </div>
                                                <div className="space-y-2">
                                                    <label className="text-xs font-bold">Title</label>
                                                    <Input value={editingBlock.title || ''} onChange={e => setEditingBlock({ ...editingBlock, title: e.target.value })} />
                                                </div>
                                            </div>
                                            <Textarea className="min-h-[100px]" value={editingBlock.content || ''} onChange={e => setEditingBlock({ ...editingBlock, content: e.target.value })} />
                                        </CardContent>
                                        <CardFooter className="gap-2">
                                    ...
                                    <Button variant="outline" className="w-full border-dashed" onClick={() => setEditingBlock({ type: 'text' })}>
