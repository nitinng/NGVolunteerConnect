...
4. **Seamless Authentication**: Utilizing Clerk for secure, password-less and social authentication, eliminating authorization bottlenecks.
...
- **Master Admin Override**: Built-in omnipresence override for top-level administrators to bypass standard constraints.
- **Automated Defaulting**: New signups are automatically provisioned with the `Volunteer` role in Clerk's metadata, securing default routes from unauthorized access.
...
- **Direct User Invites**: Send secure email invitations containing custom roles directly from the dashboard via Clerk.
...
- **Identity Provider**: [Clerk](https://clerk.com/) handling Identity, robust SSO (Single Sign-On), User Management, secure sessions, and Public Metadata (to store RBAC Roles).
...
2. **Configure Environment variables**: 
   Ensure you have configured your local `.env.local` to securely hold your Clerk API keys (`NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`) and your Master User ID (`MASTER_USER_ID`).
