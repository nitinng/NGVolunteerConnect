...
import { UsersTrendsClient } from "@/components/users/UsersTrendsClient";
import { LineChart } from "lucide-react";

export const dynamic = "force-dynamic";
export const revalidate = 0;

export default async function UsersTrendsPage() {
    ...
    return (
        <div className="flex flex-1 flex-col gap-6 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto w-full">
            <div className="relative overflow-hidden rounded-lg bg-slate-50 dark:bg-zinc-900/50 p-4 md:p-6 border border-slate-200 dark:border-zinc-800 shadow-sm">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">User Trends</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium">
                                Visualize user growth, activity, and engagement metrics.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <UsersTrendsClient initialUsers={serializedUsers} />
        </div>
    );
}
